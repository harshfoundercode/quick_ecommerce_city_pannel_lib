...
class StockProvider extends ChangeNotifier {
  ...
  void _initProducts() {
    _products = [
      ...
      Product(
        ...
        variants: [
          ProductVariant(id: 'v6', name: '8GB RAM / 256GB', sku: 'MBP-M3-8-256', stock: 15, reservedStock: 3, price: 149999),
          ProductVariant(id: 'v7', name: '16GB RAM / 512GB', sku: 'MBP-M3-16-512', stock: 7, reservedStock: 0, price: 199999),
        ],
      ),
      Product(
        id: 'p4',
  ...
  void _initIncomingStocks() {
    _incomingStocks = [
      ...
      IncomingStock(
        id: 'inc2',
        supplierName: 'FashionHub Supplies',
        ...
        items: [
          IncomingStockItem(
            ...
            receivedQty: 85,
            defectiveQty: 5,
            itemStatus: ItemStatus.defective,
          ),
          IncomingStockItem(
            id: 'ii4',
            productId: 'p4',
  ...
  List<Product> get filteredProducts {
    return _products.where((p) {
      ...
      final matchSub = _selectedSubCategoryId == null || p.subCategoryId == _selectedSubCategoryId;
  ...
  void toggleProductSelection(String productId) {
    ...
    if (idx != -1) {
      _products[idx].isSelected = !_products[idx].isSelected;
      notifyListeners();
    }
  }
  ...
  void submitBulkRequest({
    ...
    _bulkRequests.insert(0, req);
    clearSelection();
    notifyListeners();
  }

  // ─── INCOMING STOCK ACTIONS ───────────────────────────────────────
  void updateIncomingItem({
    ...
    if (allDone) {
      final hasIssues = stock.items.any(
          (i) => i.itemStatus == ItemStatus.defective || i.itemStatus == ItemStatus.missing);
      stock.status = hasIssues ? IncomingStatus.disputed : IncomingStatus.received;
    } else {
      stock.status = IncomingStatus.partiallyReceived;
    }

    // Update actual product stock
    if (status == ItemStatus.accepted || status == ItemStatus.defective) {
      final goodQty = receivedQty - defectiveQty;
      final productIdx = _products.indexWhere((p) => p.id == item.productId);
      if (productIdx != -1) {
        final variantIdx = _products[productIdx].variants.indexWhere((v) => v.id == item.variantId);
        if (variantIdx != -1) {
          _products[productIdx].variants[variantIdx].stock += goodQty;
        }
      }
    }
  ...
  void addDefectivePhoto(String incomingId, String itemId, String photoPath) {
    final stock = _incomingStocks.firstWhere((s) => s.id == incomingId);
    final item = stock.items.firstWhere((i) => i.id == itemId);
    item.defectivePhotos.add(photoPath);
    notifyListeners();
  }
}
