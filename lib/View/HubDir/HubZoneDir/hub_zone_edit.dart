...
//   final HubZoneListData zone;
//
//   /// Pass the city zone center + radius so we can validate hub location.
//   /// If your HubZoneListData already carries cityzone lat/lng/radius,
//   /// read them from zone directly inside initState instead.
...
//
//   // ── Map ───────────────────────────────────────────────────────────────────
//   // FIX: Do NOT use ValueKey on _MapPickerCard — keep a single stable map
//   // and always animate via the stored controller reference.
//   GoogleMapController? _mapController;
//   late LatLng _pickedLatLng;
...
//     _pickedLatLng = LatLng(z.latitude, z.longitude);
//
//     // ── City zone center / radius ─────────────────────────────────────────
//     // Priority: explicit widget param → fallback: read from zone model if it
//     // exposes cityzone data → last resort: use hub location as center
//     // (effectively disabling boundary check gracefully).
//     _cityCenter = widget.cityZoneCenter ?? _pickedLatLng;
//     _cityRadiusKm = widget.cityZoneRadiusKm ??
//         _tryParseZoneCityRadius(z) ??
//         double.infinity; // no limit if we can't determine
//
//     _fadeCtrl = AnimationController(
//         vsync: this, duration: const Duration(milliseconds: 500));
//     _fadeAnim = CurvedAnimation(parent: _fadeCtrl, curve: Curves.easeOut);
//     _fadeCtrl.forward();
//
//     _radiusCtrl.addListener(_rebuildCircle);
//
//     _searchFocus.addListener(() {
...
//   }
//
//   /// Try to read city zone radius from the hub zone model.
//   /// Adjust field names to match your actual model.
//   double? _tryParseZoneCityRadius(HubZoneListData z) {
//     try {
//       // e.g. z.cityZoneRadiusKm or z.cityzone?.radiuskm — adjust as needed
//       return null;
//     } catch (_) {
//       return null;
//     }
...
//         (double.tryParse(_radiusCtrl.text) ?? widget.zone.radiusInKm) * 1000;
//     return {
//       // City zone boundary (blue) — always visible
//       if (_cityRadiusKm != double.infinity)
//         Circle(
...
//   }
//
//   void _rebuildCircle() => setState(() {});
//
//   // ── FIX: _onLocationPicked — validate boundary + animate existing controller
//   void _onLocationPicked(LatLng pos) {
//     final outside = !_isInsideCity(pos);
//
//     if (outside) {
//       _showSnack(
//         '⚠ This location is outside the city zone boundary!',
//         isError: true,
//       );
//       // Still show where they tapped but keep button disabled
//       setState(() {
//         _pickedLatLng  = pos;
//         _isOutsideZone = true;
//       });
//       _latCtrl.text = pos.latitude.toStringAsFixed(6);
//       _lngCtrl.text = pos.longitude.toStringAsFixed(6);
//       // FIX: animate the EXISTING controller — do not recreate the map widget
//       _mapController?.animateCamera(CameraUpdate.newLatLng(pos));
//       return;
//     }
//
...
//     _latCtrl.text = pos.latitude.toStringAsFixed(6);
//     _lngCtrl.text = pos.longitude.toStringAsFixed(6);
//     // FIX: animate the EXISTING controller
//     _mapController?.animateCamera(CameraUpdate.newLatLng(pos));
//   }
//
//   // ── Search ────────────────────────────────────────────────────────────────
...
//   }
//
//   // FIX: _fetchPlaceDetails — properly await, then animate EXISTING controller
//   Future<void> _fetchPlaceDetails(String placeId) async {
//     _removeOverlay();
//     setState(() => _suggestions = []);
//     _searchFocus.unfocus();
//
//     // Show loading in search bar
//     setState(() => _searchLoading = true);
//
//     try {
//       final uri = Uri.parse(ApiUrl.mapPlaceDetailsUrl(placeId));
...
//       String pincode = '';
//
//       // ── Shape A: your backend structured response ─────────────────────
//       if (json['data'] != null && json['data'] is Map) {
...
//         }
//       }
//       // ── Shape B: raw Google API response ─────────────────────────────
...
//
//       if (lat == 0 && lng == 0) {
//         _showSnack('Invalid location data received.', isError: true);
//         return;
//       }
//
//       final newPos = LatLng(lat, lng);
//
//       // ── FIX: Update search bar text to show selected place ────────────
//       _searchCtrl.text = address.isNotEmpty
//           ? address
//           : '${lat.toStringAsFixed(5)}, ${lng.toStringAsFixed(5)}';
//
//       // ── FIX: Check city zone boundary BEFORE applying ─────────────────
//       final outside = !_isInsideCity(newPos);
//
//       if (outside) {
//         // Still move the map so user can see where the place is
//         setState(() {
...
//
//         // FIX: animate EXISTING controller (no ValueKey rebuild)
...
//         _showSnack(
//           'This location is outside the city zone boundary. '
//               'Please select a location inside the blue circle.',
//           isError: true,
//         );
//         return;
//       }
//
//       // ── All good — apply location ─────────────────────────────────────
//       setState(() {
//         _pickedLatLng  = newPos;
//         _isOutsideZone = false;
//       });
...
//
//       // FIX: animate the EXISTING map controller — this is the key fix
//       // The old code called _mapController?.animateCamera BEFORE setState
//       // finished, and the ValueKey rebuild destroyed the controller.
//       await _mapController?.animateCamera(
//         CameraUpdate.newCameraPosition(
...
//     if (_isOutsideZone) {
//       _showSnack(
//         'Cannot save — location is outside the city zone boundary.',
//         isError: true,
//       );
//       return;
//     }
...
//     FocusScope.of(context).unfocus();
//     final hubZone =
//     Provider.of<HubZoneEditViewModel>(context, listen: false);
//     hubZone.editZoneApi(
//       context,
//       widget.zone.id.toString(),
//       widget.zone.cityzoneid.toString(),
...
//                             label: 'Search Location',
//                             subtitle:
//                             'Search inside the city zone to auto-fill map & address',
//                           ),
//
//                           Padding(
//                             padding:
...
//                           ),
//
//                           // FIX: NO ValueKey — stable widget, stable controller
//                           _MapPickerCard(
//                             initialPosition: _pickedLatLng,
//                             markers: _markers,
//                             circles: _circles,
//                             onMapCreated: (c) => _mapController = c,
//                             onTap: _onLocationPicked,
//                             cityCenter: _cityRadiusKm != double.infinity
//                                 ? _cityCenter
//                                 : null,
//                           ),
//
...
//                             icon: Icons.hub_rounded,
//                             label: 'Zone Details',
//                             subtitle: 'Basic information about this hub zone',
//                           ),
//
//                           Padding(
//                             padding: const EdgeInsets.symmetric(
...
//             Text('Outside City Zone Boundary',
//                 style: TextStyle(
//                     fontSize: 12,
//                     fontWeight: FontWeight.w700,
...
//             const SizedBox(height: 2),
//             Text(
//               'This location is outside the allowed city zone. '
...
//                 'Selecting a suggestion fills map, coordinates & address.',
//             style: TextStyle(
//                 fontSize: 11, color: Color(0xFF065F46), height: 1.5),
//           ),
//         ),
//       ]),
//     );
...
//
// // ── Map Picker Card ───────────────────────────────────────────────────────────
// // FIX: Removed ValueKey — this widget must stay STABLE so the
// // GoogleMapController reference is never destroyed.
//
// class _MapPickerCard extends StatelessWidget {
//   final LatLng initialPosition;
//   final Set<Marker> markers;
...
//   final void Function(GoogleMapController) onMapCreated;
//   final void Function(LatLng) onTap;
//   final LatLng? cityCenter; // for legend chip
//
//   const _MapPickerCard({
//     required this.initialPosition,
//     required this.markers,
...
//                 Icon(Icons.touch_app_rounded, size: 13, color: _kAccent),
//                 SizedBox(width: 4),
//                 Text('Tap or drag pin',
//                     style: TextStyle(
//                         fontSize: 11,
...
//   @override
//   Widget build(BuildContext context) {
//     final blocked = isLoading || isDisabled;
//     return SizedBox(
//       width: double.infinity,
//       height: 52,
//       child: ElevatedButton(
...
//           isDisabled ? Colors.red.shade400 : _kAccent.withValues(alpha: 0.6),
...
//             const SizedBox(width: 8),
//             Text(
//               isDisabled ? 'Outside City Zone' : 'Save Changes',
//               style: const TextStyle(
//                   fontSize: 15, fontWeight: FontWeight.w700),
//             ),
//           ],
...
class _HubZoneEditScreenState extends State<HubZoneEditScreen>
  ...
  void initState() {
    ...
    _pickedLatLng = LatLng(z.latitude, z.longitude);
    _cityCenter   = widget.cityZoneCenter ?? _pickedLatLng;
    _cityRadiusKm = widget.cityZoneRadiusKm ?? double.infinity;
