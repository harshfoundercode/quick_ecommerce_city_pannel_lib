...
class _HubZoneEditScreenState extends State<HubZoneEditScreen>
  ...
  double _distanceKm(LatLng a, LatLng b) {
    ...
    final x =
        s * s + cos(_deg2rad(a.latitude)) * cos(_deg2rad(b.latitude)) * t * t;
  ...
  void _onLocationPicked(LatLng pos) {
    ...
    if (outside) {
      _showSnack(
        '❌ Location is outside the city zone! Move pin inside the blue circle.',
        isError: true,
      );
    }
  }
          ...
          child: Consumer<HubZoneEditViewModel>(
            builder: (context, evm, _) => LayoutBuilder(
              builder: (context, constraints) {
                ...
                return isWeb ? _buildWebLayout(evm) : _buildMobileLayout(evm);
              },
            ),
          ),
        ),
            ...
            child: Column(
              ...
              children: [
                ...
                AnimatedSize(
                  ...
                  child:
                      (_showOutsideBanner && _cityRadiusKm != double.infinity)
                      ? Padding(
...
class _DropdownList extends StatelessWidget {
  ...
  const _DropdownList({required this.suggestions, required this.onSelect});
...
class _MapChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = textColor ?? (isWarning ? const Color(0xFFEF4444) : _kAccent);
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: isLoading
            ...
            : Row(
                ...
                children: [
                  Icon(
                    isDisabled ? Icons.block_rounded : Icons.save_rounded,
                    size: 20,
                  ),
                  const SizedBox(width: 8),
                  Text(
