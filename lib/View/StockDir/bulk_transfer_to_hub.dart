...
class _BulkTransferScreenState extends State<BulkTransferScreen> {
  ...
  void _toggle(CityStockData item) {
    ...
    if (id == null || (item.totalStock ?? 0) == 0) return;
    setState(() {
                              ...
                              Row(children: [
                                ...
                                if (item.mainCategoryName != null) ...[
                                  ...
                                  Container(
                                    ...
                                    child: Text(item.categoryName!, style: const TextStyle(fontSize: 9, color: Color(0xFF6B7280))),
                                  ),
                                ],
                              ]),
                            ])),
      ...
      itemBuilder: (_, i) {
        ...
        return Container(
          ...
          child: Row(children: [
            ...
            // Product info
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.productName ?? '', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF111827)),
        ...
        child: ElevatedButton.icon(
          onPressed: (_canConfirm && !vm.transferLoading) ? () {
            final items = _selectedQty.entries.map((e) {
              ...
              return {'productid': e.key, 'variantid': product?.variants?.first ?? 0, 'qty': e.value};
            }).toList();
