...
class _MapPickerPopupState extends State<MapPickerPopup>
  ...
  // ── KEY FIX: controls the transparent map blocker ─────────────────────────
  // When true, a transparent GestureDetector sits on top of the GoogleMap
  // platform view so all pointer events are consumed before reaching the map.
  bool _isDropdownOpen = false;
  ...
  // ── Dropdown open / close (single source of truth) ───────────────────────

  /// Opens the dropdown. On web uses OverlayEntry; on mobile uses setState.
  void _openDropdown() {
  ...
  /// Closes dropdown, removes overlay, resets the blocker flag.
  void _closeDropdown() {
    _removeDropdownOverlay();
    if (mounted) {
      setState(() {
      ...
      builder: (_) => Positioned(
        ...
        child: Material(
          ...
          child: _WebSearchDropdownList(
            results: List.from(_searchResults),
            onSelect: (placeId) {
              // Set flag BEFORE any async work so map tap guard is already up
              _isSelectingFromSearch = true;
              _closeDropdown();
              _selectPlace(placeId);
            },
  ...
  Future<void> _searchPlaces(String q) async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _searchResults = results;
          // Show blocker whenever results are visible
    ...
    return Row(
      children: [
        Expanded(
          ...
          child: Stack(
            children: [
              // ✅ FIX: Use _buildMapWithBlocker() — not _buildGoogleMap() directly
              ...
              Positioned(
                ...
                child: Column(
                  children: [
                    _buildSearchBar(key: _searchBarKey),
                    // Web dropdown is in OverlayEntry, not here.
                    // Mobile dropdown shown below search bar inline.
                    if (!kIsWeb && _searchResults.isNotEmpty) ...[
                      const SizedBox(height: 4),
                      _buildSearchDropdown(onSelect: (placeId) {
                        _isSelectingFromSearch = true;
                        _selectPlace(placeId);
                      }),
                    ],
              ...
              child: Stack(
                children: [
                  Padding(
                    ...
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(14),
                      // ✅ FIX: Use _buildMapWithBlocker() here too
  ...
  // ── ✅ KEY FIX: Map + transparent blocker overlay ─────────────────────────
  //
  // GoogleMap is a platform view (AndroidView / UiKitView / HtmlElementView).
  // Platform views receive pointer events at OS level, bypassing Flutter's
  // hit-test tree completely. AbsorbPointer / IgnorePointer have NO effect.
  //
  // The ONLY reliable solution is to place a transparent Flutter widget on top
  // of the platform view. When _isDropdownOpen is true this widget covers the
  // entire map and intercepts every pointer event before it reaches the native
  // view. The map never sees the touch, so no marker moves, no tap fires.
    ...
    return Stack(
      children: [
        ...
        if (_isDropdownOpen)
          Positioned.fill(
            child: GestureDetector(
              // opaque = absorbs even taps that land on empty space
              behavior: HitTestBehavior.opaque,
              onTap: () {
                // Tapping the map area while dropdown is open → just close it
                _closeDropdown();
                _searchFocus.unfocus();
              },
              // Absorb all other gestures so map pan/zoom don't fire either
              onPanDown: (_) {},
              onScaleStart: (_) {},
  ...
  Widget _buildGoogleMap() {
    return GoogleMap(
      ...
      gestureRecognizers: kIsWeb
          ? (_isDropdownOpen
      // Dropdown open → give up gesture priority so the
      // transparent blocker above can intercept cleanly
          ? <Factory<OneSequenceGestureRecognizer>>{}
      // Dropdown closed → EagerGestureRecognizer for normal map use
          : <Factory<OneSequenceGestureRecognizer>>{
  ...
  Widget _buildConfirmButton() {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor:
          _isOutsideBoundary ? Colors.red.shade400 : ColorConst.primaryGreen,
          disabledBackgroundColor: Colors.red.shade400,
          foregroundColor: Colors.white,
          disabledForegroundColor: Colors.white,
          elevation: 0,
