...
//           return;
//         }
//         // _onMapTap ke andar, setState se pehle:
//         if (!_isHubInsideCity(pos, _hubRadius)) {
//           // Hub radius city se bahar jayega — radius auto-clamp karo
//           final maxAllowed = (_cityRadiusKm - _distanceKm(pos, _cityCenter))
//               .clamp(0.5, _cityRadiusKm);
//           setState(() {
//             _selectedLocation = pos;
//             _hubRadius = maxAllowed;
//             _radiusCtrl.text = maxAllowed.toStringAsFixed(1);
//             _isOutsideBoundary = false;
//             _searchResultOutside = false;
//           });
//           CustomSnackBar.show(
//             context,
//             message: '⚠️ Radius adjusted to fit within city boundary.',
...
//     }
//     // _onMapTap ke andar, setState se pehle:
//     if (!_isHubInsideCity(latLng, _hubRadius)) {
//       // Hub radius city se bahar jayega — radius auto-clamp karo
//       final maxAllowed = (_cityRadiusKm - _distanceKm(latLng, _cityCenter))
//           .clamp(0.5, _cityRadiusKm);
//       setState(() {
...
//     }
//   }
//   bool _isRadiusExceeding = false; // NEW
//
//   bool _isHubFullyInsideCity(LatLng center, double radiusKm) {
//     return _distanceKm(center, _cityCenter) + radiusKm <= _cityRadiusKm;
//   }
//
//   bool _isHubInsideCity(LatLng center, double radiusKm) {
//     // Hub ka koi bhi edge city boundary ke bahar nahi jana chahiye
//     // Distance from city center to hub center + hub radius <= city radius
//     return _distanceKm(center, _cityCenter) + radiusKm <= _cityRadiusKm;
//   }
//   // ── Radius ────────────────────────────────────────────────────────────────
//
...
//       return;
//     }
//     if (!_isHubInsideCity(_selectedLocation, val)) {
//       CustomSnackBar.show(
//         context,
//         message: '❌ Radius exceeds city boundary!',
//         type: SnackBarType.error,
//       );
//       return;
//     }
...
//       if (!_isHubInsideCity(_selectedLocation, parsed)) {
//         CustomSnackBar.show(
//           context,
//           message: '❌ Radius would go outside the city boundary!',
//           type: SnackBarType.error,
//         );
//         return;
//       }
...
class _MapPickerPopupState extends State<MapPickerPopup>
  ...
  // ── NEW: radius exceeds city boundary flag ────────────────────────────────
  bool _isRadiusExceeding = false;
  ...
  // ── NEW: checks if hub circle (center + radius) is fully inside city ──────
  // Formula: distance(hubCenter, cityCenter) + hubRadius <= cityRadius
  bool _isHubFullyInsideCity(LatLng center, double radiusKm) {
    return _distanceKm(center, _cityCenter) + radiusKm <= _cityRadiusKm;
  }
  ...
  Set<Circle> get _circles {
    final hubVM = Provider.of<HubZoneViewModel>(context, listen: false);
    // Hub circle is invalid if pin is outside boundary OR radius exceeds city
    final bool hubInvalid = _isOutsideBoundary || _isRadiusExceeding;
    return {
      Circle(
        circleId: const CircleId('city_boundary'),
        center: _cityCenter,
      ...
      Circle(
        ...
        center: _selectedLocation,
        radius: _hubRadius * 1000,
        fillColor: hubInvalid
            ? Colors.red.withValues(alpha: 0.10)
            : ColorConst.primaryGreen.withValues(alpha: 0.12),
        strokeColor: hubInvalid
            ? Colors.red.withValues(alpha: 0.6)
            : ColorConst.primaryGreen.withValues(alpha: 0.7),
        strokeWidth: 2,
      ),
  ...
  Set<Marker> get _markers => {
    ...
    Marker(
      ...
      draggable: true,
      icon: BitmapDescriptor.defaultMarkerWithHue(
        (_isOutsideBoundary || _isRadiusExceeding)
            ? BitmapDescriptor.hueRed
            : BitmapDescriptor.hueGreen,
      ),
      infoWindow: InfoWindow(
        ...
        snippet: _isOutsideBoundary
            ? '⚠️ Outside city boundary'
            : _isRadiusExceeding
            ? '⚠️ Radius exceeds city boundary'
            : '✓ Valid location\nRadius: ${_hubRadius.toStringAsFixed(1)} km',
      ),
      onDragStart: (_) => debugPrint('Dragging started'),
      onDrag: (pos) {
      ...
      onDragEnd: (pos) async {
        ...
        // Check if current radius still fits after drag
        final bool radiusStillFits = _isHubFullyInsideCity(pos, _hubRadius);
        if (!radiusStillFits) {
          // Auto-clamp radius to max allowed at this position
          final maxAllowed =
          (_cityRadiusKm - _distanceKm(pos, _cityCenter))
              .clamp(0.5, _cityRadiusKm);
          setState(() {
            _selectedLocation = pos;
            _hubRadius = maxAllowed;
        ...
        setState(() {
          _selectedLocation = pos;
          _isOutsideBoundary = !_isInsideCity(pos);
          _isRadiusExceeding = !_isHubFullyInsideCity(pos, _hubRadius);
        });
        await _fetchAddress(pos);
        _mapController?.animateCamera(CameraUpdate.newLatLng(pos));
      },
  ...
  Future<void> _onMapTap(LatLng latLng) async {
    ...
    // Check if current radius fits at the new tap location
    if (!_isHubFullyInsideCity(latLng, _hubRadius)) {
      // Auto-clamp radius to max allowed at this position
      final maxAllowed = (_cityRadiusKm - _distanceKm(latLng, _cityCenter))
          .clamp(0.5, _cityRadiusKm);
      setState(() {
    ...
    setState(() {
      _selectedLocation = latLng;
      _isOutsideBoundary = !_isInsideCity(latLng);
      _isRadiusExceeding = !_isHubFullyInsideCity(latLng, _hubRadius);
      _searchResultOutside = false;
    });
    _fetchAddress(latLng);
    _mapController?.animateCamera(CameraUpdate.newLatLng(latLng));
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        best ??= results.firstWhere(
          ...
          orElse: () => results.firstWhere(
                (r) =>
                (r['types'] as List).contains('administrative_area_level_2'),
            orElse: () => results.first,
          ),
        );
  ...
  Future<void> _selectPlace(String placeId, {String description = ''}) async {
    ...
    try {
      ...
      if (outside) {
        if (mounted) {
          setState(() {
            _selectedLocation = latLng;
            _isOutsideBoundary = true;
            _isRadiusExceeding = false; // pin itself is outside, show that error
            _searchResultOutside = true;
            _searchOutsideMsg = 'This place is outside the city zone '
                '"${widget.cityZone?.name ?? 'boundary'}". '
                'Only locations inside the blue circle are allowed.';
      ...
      // Check if current radius fits at this new search location
      final bool radiusFits = _isHubFullyInsideCity(latLng, _hubRadius);

      if (mounted) {
        if (!radiusFits) {
          // Radius exceeds — warn user, block confirm until they fix radius
          setState(() {
            _selectedLocation = latLng;
            _isOutsideBoundary = false;
            _searchResultOutside = false;
            _searchOutsideMsg = '';
            _isRadiusExceeding = true;
          });
          CustomSnackBar.show(
            context,
            message:
            '⚠️ Location selected but current radius exceeds city boundary. Please reduce the radius to confirm.',
            type: SnackBarType.warning,
          );
        } else {
          setState(() {
  ...
  void _onRadiusSlider(double val) {
    ...
    // Check if new radius stays within city boundary
    final bool fits = _isHubFullyInsideCity(_selectedLocation, val);
    if (!fits) {
      CustomSnackBar.show(
        context,
        message: '❌ Radius exceeds city boundary!',
      ...
      // Still update radius visually so user sees the red circle,
      // but mark _isRadiusExceeding = true to block confirm
    }
    setState(() {
      _hubRadius = val;
      _radiusCtrl.text = val.toStringAsFixed(1);
      _isRadiusExceeding = !fits;
    });
    _fitHubCoverage();
  }
  ...
  void _onRadiusField(String val) {
    ...
    if (parsed != null && parsed >= 0.5 && parsed <= _cityRadiusKm) {
      ...
      final bool fits = _isHubFullyInsideCity(_selectedLocation, parsed);
      if (!fits) {
        CustomSnackBar.show(
          context,
          message: '❌ Radius would go outside the city boundary!',
  ...
  void _confirm() {
    // Hard block — pin outside boundary
    if (_isOutsideBoundary) return;
    // Hard block — radius exceeds city boundary
    if (_isRadiusExceeding) {
      CustomSnackBar.show(
        context,
        message:
        '❌ Radius exceeds city boundary. Please reduce the radius before confirming.',
        type: SnackBarType.error,
      );
      return;
    }
                  ...
                  children: [
                    _HintChip(
                      label: _isOutsideBoundary
                          ? '⚠ Outside boundary'
                          : _isRadiusExceeding
                          ? '⚠ Radius exceeds boundary'
                          : 'Click inside the blue circle',
                      isWarning: _isOutsideBoundary || _isRadiusExceeding,
                    ),
                    const SizedBox(height: 6),
                    const _LegendChip(
                      color: Color(0xFFFB923C),
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  if (_isRadiusExceeding && !_isOutsideBoundary && !_searchResultOutside) ...[
                    const SizedBox(height: 4),
                    _buildRadiusExceedingWarning(),
                  ],
                  if (_isOutsideBoundary && !_searchResultOutside) ...[
                    const SizedBox(height: 4),
                    _buildOutsideWarning(),
          ...
          children: [
            Column(
              children: [
                ...
                // NEW: show radius exceeding warning in right panel too
                if (_isRadiusExceeding && !_isOutsideBoundary && !_searchResultOutside) ...[
                  const SizedBox(height: 4),
                  _buildRadiusExceedingWarning(),
                ],
        ...
        Row(
          children: [
            Expanded(
              child: SliderTheme(
                data: SliderThemeData(
                  activeTrackColor: _isRadiusExceeding
                      ? Colors.red.shade400
                      : ColorConst.primaryGreen,
                  inactiveTrackColor: const Color(0xFFE5E7EB),
                  thumbColor: _isRadiusExceeding
                      ? Colors.red.shade400
                      : ColorConst.primaryGreen,
                  overlayColor: _isRadiusExceeding
  ...
  // ── NEW: Radius exceeding warning widget ──────────────────────────────────

  Widget _buildRadiusExceedingWarning() {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          Icon(Icons.radar_rounded, size: 16, color: Colors.orange.shade700),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              'Radius exceeds city boundary — reduce it to confirm.',
              style: TextStyle(
                  fontSize: 12,
          ...
          GestureDetector(
            onTap: () {
              // Auto-set radius to max allowed at current location
              final maxAllowed = (_cityRadiusKm -
                  _distanceKm(_selectedLocation, _cityCenter))
                  .clamp(0.5, _cityRadiusKm);
              setState(() {
                _hubRadius = maxAllowed;
                _radiusCtrl.text = maxAllowed.toStringAsFixed(1);
  ...
  Widget _buildConfirmButton() {
    // Button is blocked if pin is outside OR radius exceeds boundary
    final bool blocked = _isOutsideBoundary || _isRadiusExceeding;
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: blocked ? Colors.red.shade400 : ColorConst.primaryGreen,
          disabledBackgroundColor: Colors.red.shade400,
          foregroundColor: Colors.white,
          disabledForegroundColor: Colors.white,
          elevation: 0,
